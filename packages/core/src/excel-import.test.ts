...
describe('planDictImport — 단어', () => {
  it('신규 단어를 파싱한다', () => {
    ...
    expect(plan.entries).toEqual([
      { kind: 'word', row: 1, existingId: null, draft: { ...draft, origin: null }, patch: draft },
    ])
...
describe('planDictImport — 도메인', () => {
  it('방언별 타입·허용값을 파싱한다', () => {
    ...
    expect(plan.entries[0]).toEqual({
      kind: 'domain', row: 1, existingId: null, draft: { ...draft, origin: null }, patch: draft,
    })
  })
...
describe('planDictImport — 용어', () => {
  it('구성 단어 컬럼은 무시하고 물리명·기본 도메인을 읽는다', () => {
    ...
    expect(plan.entries[0]).toEqual({
      kind: 'term', row: 1, existingId: null, draft: { ...draft, origin: null }, patch: draft,
    })
    expect(plan.issues).toHaveLength(0)
  })
...
describe('planDictImport — 인식 컬럼과 부분 갱신값(patch)', () => {
  ...
  it('없는 컬럼은 patch에서 빠지고 draft에만 기본값으로 들어간다', () => {
    ...
    expect(e.draft).toEqual({
      logicalName: '주문', abbreviation: 'ORD', englishName: null, description: null, origin: null,
    })
  })
