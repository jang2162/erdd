...
describe('generatePhysicalName', () => {
  ...
  it('lower_snake·구분자 없음', () => {
    expect(generatePhysicalName('회원상태', words, {}, { case:'lower_snake', separator:'_', logicalSeparator:'_', maxLengthBytes:30, tablePhysicalTemplate:'', tableLogicalTemplate:'' }).physicalName).toBe('mbr_stat')
    expect(generatePhysicalName('회원상태', words, {}, { case:'UPPER_SNAKE', separator:'', logicalSeparator:'_', maxLengthBytes:30, tablePhysicalTemplate:'', tableLogicalTemplate:'' }).physicalName).toBe('MBRSTAT')
  })
})
...
describe('suggestCompletions', () => {
  ...
  it('물리명 — 구분자가 없는 규칙에서는 약어 그리디로 끊고 남은 꼬리가 쿼리다', () => {
    const rules = { case: 'UPPER_SNAKE' as const, separator: '' as const, logicalSeparator: '_' as const, maxLengthBytes: 30, tablePhysicalTemplate: '', tableLogicalTemplate: '' }
