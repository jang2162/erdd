...
/**
 * 인용 구간 — 문자열 리터럴(`'…'`)과 따옴표 식별자(`"…"`, `` `…` ``, `[…]`) — 을 여는·닫는
 * 따옴표까지 통째로 같은 길이의 자리표시 문자로 덮는다. 키워드 스캔이 코멘트·기본값 안의
 * 단어나 `"unique"` 같은 식별자를 구조 키워드로 오인하지 않게 하는 장치다
 * (사용자 DDL의 `model_indexes."unique" boolean not null`이 실제 사례다).
 *
 * **네 종류를 한 번의 좌→우 스캔에서 함께 처리하는 것이 핵심이다.** 리터럴과 식별자를 각각
 * 독립된 스캔으로 돌리면 어느 쪽을 먼저 돌려도 반대편에 구멍이 생긴다 — 리터럴이 먼저면
 * `"o'brien"`의 아포스트로피를 문자열 시작으로 오인해 뒤를 통째로 덮고, 식별자가 먼저면
 * `DEFAULT '{"a": 1}'`(PostgreSQL jsonb)의 큰따옴표를 식별자 시작으로 오인한다. 이 파일의
 * splitStatements·splitTopLevel·unquoteIdentifier가 모두 같은 관례를 따른다.
 *
 * 길이를 보존하므로 마스킹본에서 찾은 인덱스를 원본에 그대로 쓸 수 있다(코드 유닛 단위 순회 —
 * 서로게이트 페어를 한 글자로 묶으면 길이가 줄어 뒤쪽 인덱스가 어긋난다).
 *
...
function maskQuoted(text: string): string {
  ...
  for (let i = 0; i < text.length; i++) {
    const c = text[i]!
    const close = c === "'" || c === '"' || c === '`' ? c : c === '[' ? ']' : null
    if (close === null) { out += c; continue }
    out += FILL
    i++
    while (i < text.length) {
...
/**
 * 구조 키워드의 **위치**를 찾기 위한 사본을 만든다. 인용 구간과 괄호 안 내용을 자리표시로
 * 덮는다. 값은 언제나 원본에서 잘라낸다(이 파일의 관례 — parseColumnDef와 MySQL 꼬리
 * COMMENT가 이미 같은 방식이다). 길이가 보존되므로 인덱스를 그대로 공유한다.
 *
 * **두 단계의 순서는 바꿀 수 없다.** 괄호 깊이는 인용이 전부 덮인 뒤에 세야 한다 — 그래야
 * 리터럴·식별자 안의 괄호(`DEFAULT '(1)'`, `COLLATE "a(b"`)가 깊이를 어긋나게 하지 않는다.
 * (인용 4종 사이에는 순서가 없다 — maskQuoted가 한 번의 스캔으로 처리한다.)
 */
...
function parseColumnDef(def: string): { column: ParsedColumn; attrs: string } | null {
  ...
  // 구조 키워드(NOT NULL·PRIMARY KEY·자동증가 등) 판정은 인용 구간 안의 내용에 속으면
  // 안 되므로 그것을 덮은 마스킹본에서 찾는다. comment 내용 자체가 필요한 commentMatch만
  // 예외적으로 원본 attrs를 그대로 쓴다.
  const attrsScan = maskQuoted(attrs)
  ...
  const maskedForDefault = maskQuoted(attrsForDefault)
...
function parseCreateTable(
  ...
  // 마스킹은 인용 구간의 내용뿐 아니라 여는/닫는 따옴표까지 자리표시로 덮으므로(maskQuoted
