...
> 끌어오지 않으므로** 소비처가 자기 프로젝트에 직접 선언해야 한다. 빠뜨리면 CLI 가 아니라 셸이
> `sh: tsx: command not found`(종료 코드 `127`)로 끝낸다.
...
| 배치 좌표·메모 | `erdd/layout.yaml` (커밋한다) | 서버 DB (파일에 담기지 않는다) |
...
| 명령 | | 모드 |
|---|---|---|
| `erdd init` | 서버에 연결한다 (URL·토큰·프로젝트) | 서버 |
...
| `erdd diff` | 올릴 변경·내려올 변경·충돌을 미리 본다 | **서버 전용** |
