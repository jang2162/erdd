...
// 깨진다. 그래도 절대 URL 로 바꾸지 않는다 — 저장소 안에서 매일 쓰는 클릭 이동을 잃고, 사내 GitLab
// 주소가 바뀌면 그때 깨진다. 그런 링크는 기여자용이라 패키지를 설치한 사람이 볼 일이 없어 **알고
// 그대로 둔다.** 매뉴얼에 manual 밖을 가리키는 링크를 새로 넣으면 같은 일이 생긴다는 것만 알고
// 넣어라. 정본은 `docs/guides/release.md` 의 「번들되는 것 → 매뉴얼」 절이다.
//
