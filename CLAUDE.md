...
## 작업을 다른 에이전트에 맡길 때는 Orca 환경이면 `/orchestration` 로

**병렬이든 순차든 상관없다.** 워크트리를 쓰거나 작업 일부를 다른 에이전트에 맡기는 상황이면, 먼저
**Orca 앱 환경인지 확인**하고 가능하면 **Orca 오케스트레이션(`/orchestration`)으로 별도 Orca 터미널에
워커를 띄워** 진행한다 — 백그라운드 서브에이전트(Agent 도구)보다 우선한다.

- **적용 범위에 예외를 만들지 마라.** "단일 워크트리라서", "태스크를 순차로 도니까", "서브에이전트
  하나뿐이라서"는 이 규칙을 건너뛸 이유가 **아니다**. 판단 기준은 병렬성이 아니라 **작업이 이 세션
  밖의 에이전트에게 넘어가는가**이다. SDD(subagent-driven-development)처럼 태스크마다 워커를 띄우는
  워크플로도 여기 해당한다 — 구현자·리뷰어를 Orca 터미널 워커로 띄운다.
  (2026-08-04 실제 발생: 승격 요청 큐 사이클에서 "단일 워크트리 + 순차 태스크라 병렬이 아니다"라고
  판단해 Agent 도구로 진행했다. 사용자가 바로잡았고, 그래서 이 규칙의 조건을 병렬성에서 위임 여부로
  바꿨다.)
