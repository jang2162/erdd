...
이 파일은 **매 세션 반드시 지켜야 할 규칙**만 담는다. 제품 사용법은 **`docs/manual/`**, 기능별 설계
근거와 결정 기록은 **`docs/superpowers/specs/`** 에 있다.
...
- **How to apply:** 사용자가 "기억해"라고 하거나 재사용할 결정이 확정되면 → 저장소 안의 지침 문서에
  추가하고 커밋한다. 이 저장소에서 목적지는 보통 둘 중 하나다.
  | 기록할 것 | 목적지 |
  |---|---|
  | 반드시 지켜야 할 작업 규칙 | 이 파일(`CLAUDE.md`) |
  | 설계 근거·기획 결정 | `docs/superpowers/specs/` 의 해당 설계 문서 |
...
- **예외는 확장이 하나뿐인 브라우저 스모크다**. CLI 스모크처럼 터미널로 끝나는
  것은 에이전트에게 시킨다.
