...
describe('useDragStore', () => {
  ...
  it('start는 넘겨받은 배열을 복사한다 — 호출자의 선택 배열과 엮이지 않는다', () => {
    const ids = ['t1']
    useDragStore.getState().start(ids, 'sidebar')
    expect(useDragStore.getState().tableIds).not.toBe(ids)
    expect(useDragStore.getState().tableIds).toEqual(['t1'])
  })

  it('소스를 기록하고 end가 지운다 — 커서 고스트가 이 값으로 갈린다', () => {
    // `tableIds`·`over`로는 두 소스를 구분할 수 없다(모양이 같다). 고스트는 사이드바 드래그에만
    // 떠야 하므로 시작할 때 어디서 왔는지 명시적으로 남긴다.
    useDragStore.getState().start(['t1'], 'canvas')
    expect(useDragStore.getState().source).toBe('canvas')
    useDragStore.getState().end()
    expect(useDragStore.getState().source).toBeNull()
  })
})
