...
describe('one-time-token', () => {
  ...
  it('살아 있는 토큰은 통과한다', () => {
    expect(() => assertLive({ expiresAt: new Date(Date.now() + 60_000), usedAt: null }, 'invitation')).not.toThrow()
  })
  ...
  it('만료된 토큰과 사용된 토큰을 구분해 거부한다', () => {
    ...
    expect(() => assertLive({ expiresAt: new Date(Date.now() - 1), usedAt: null }, 'invitation'))
      .toThrow('기한')
    expect(() => assertLive({ expiresAt: new Date(Date.now() + 60_000), usedAt: new Date() }, 'invitation'))
      .toThrow('사용')
  })

  it('사용된 뒤 기한까지 지난 토큰은 "이미 사용"으로 안내한다', () => {
    // 두 조건이 동시에 참인 유일한 입력이다 — 검사 순서가 뒤집히면 여기서만 갈린다.
    expect(() => assertLive({ expiresAt: new Date(Date.now() - 1), usedAt: new Date() }, 'invitation'))
      .toThrow('사용')
  })
})
