...
export class LinkDeadError extends TRPCError {
  /**
   * **이 링크를 다시 발급받을 수 있는가.** 화면의 다음 행동이 여기서 갈린다 —
   * `true`면 "관리자에게 문의해 새 링크를 받으세요", `false`면 로그인을 가리킨다(설계 §6.1).
   * 응답에는 `data.linkReissuable`로 나간다.
   *
   * **기본값이 `true`인 것이 의도다.** 표식은 "다시 받을 수 없다"는 **더 센 주장** 쪽에만
   * 명시적으로 단다 — 새 사유가 추가되면 자동으로 약한 주장("관리자에게 문의")으로 떨어지고,
   * 그것은 늘 참인 안내다(관리자는 적어도 사람이다). 반대로 기본값을 `false`로 두면 새 사유가
   * 조용히 "새 링크를 받을 수 없습니다"라고 단정하게 되는데, 그것이 거짓이면 사용자는 실제로
   * 가능한 회복 경로를 포기한다. 오분류의 대가가 한쪽으로만 크다는 §6.1의 원칙 그대로다.
   *
   * 코드로는 갈릴 수 없다 — 소비된 초대(`BAD_REQUEST`)와 만료된 초대(`BAD_REQUEST`)가 같은
   * 코드로 오고 message만 다르다(실측 2026-08-09). message 문자열로 갈래를 판정하는 것은
   * `linkDead`에서 이미 버린 길이다(문구가 바뀌면 조용히 오분류된다).
   */
  ...
  constructor(opts: {
    ...
    message: string
    /** 생략하면 `true` — 안전한 쪽. 위 필드 주석 참조. */
    reissuable?: boolean
  }) {
    super(opts)
    this.reissuable = opts.reissuable ?? true
  }
}
 ...
 *
 * **재발급 가능 여부는 종류에 따라 갈리므로 `kind`를 받는다.**
 * - **초대가 사용됨 ⇒ 재발급 불가.** `accept`는 계정 생성과 `usedAt` 설정이 한 트랜잭션이라,
 *   `usedAt`이 있으면 그 이메일의 계정이 **반드시 만들어졌다**. 그러면 새 초대는 존재할 수 없다 —
 *   `admin.users.invite`와 `invitation.create`가 둘 다 409로 거절한다(실측 2026-08-09).
 * - **재설정이 사용됨 ⇒ 재발급 가능.** 계정이 있으니 관리자가 `admin.users.resetLink`로 새 링크를
 *   낼 수 있다(실측 2026-08-09: 200).
 * - **기한이 지남 ⇒ 종류를 불문하고 재발급 가능.** 초대의 경우 계정이 아직 없을 수 있고, 없으면
 *   관리자가 새 초대를 만들 수 있다(실측: 취소된 초대의 이메일로 재발급 200). 취소도 이 갈래다 —
 *   서버가 `expiresAt`을 당기는 것이라 만료로 온다.
 */
export function assertLive(row: { expiresAt: Date; usedAt: Date | null }, kind: TokenKind): void {
  if (row.usedAt !== null) {
    throw new LinkDeadError({
      code: 'BAD_REQUEST', message: '이미 사용된 링크입니다',
      reissuable: kind !== 'invitation',
    })
  }
