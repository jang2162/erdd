...
패키지 `@erdd/cli`(바이너리 `erdd`)와 그것이 의존하는 `@erdd/core` 를 **사내 GitLab(`gitlab.develma.com`)의 패키지 레지스트리**에 게시한다. 공개 npm 에는 올리지 않는다 — 소비처가 전부 사내 저장소이고, 스코프 `@erdd` 만 사내 레지스트리로 보내는 `.npmrc` 한 줄로 끝나기 때문이다. 설치 절차는 [CLI 매뉴얼 2절](manual/cli-guide.md#2-설치와-실행), 게시하는 쪽의 절차는 [HANDOFF 4.1절](superpowers/HANDOFF.md#41-릴리스--사내-레지스트리에-cli-패키지를-게시한다)에 있다.

게시 형태를 정하는 결정 넷과 그 이유.

- **원본 TypeScript 를 그대로 배포한다(빌드 산출물이 아니다).** 진입점의 shebang 이 `#!/usr/bin/env -S npx tsx`라 소비처에 `tsx`가 필요하지만, 그 대가로 번들링·소스맵·이중 진입점을 관리하지 않아도 되고 **저장소에서 직접 실행하던 형태와 게시본이 같은 코드**가 된다. CLI 는 사내 개발자가 쓰는 도구라 `tsx` 하나를 더 요구하는 비용이 작다. **`tsx` 는 소비처가 자기 프로젝트에 직접 선언한다** — `@erdd/cli` 의 `dependencies` 에 넣어 대신 해결하려던 시도는 **pnpm 소비처를 회귀시켜 되돌렸다**(선언이 `npx` 에게 「이미 설치됨」으로 오판을 시키는데 pnpm 은 그 bin 을 어떤 `.bin` 에도 링크하지 않는다 — 되던 것이 종료 코드 127 이 됐다). 근본 해결은 셰방을 `node` 런처로 바꾸는 것이고 후속 항목으로 올려 뒀다(→ [HANDOFF 4.1b](superpowers/HANDOFF.md#41b-게시-관련-이월-항목-일부러-고치지-않은-것)). 두 패키지 모두 `engines.node: ">=22"` 로 Node 버전을 소비처에 강제한다.
- **웹 번들을 tarball 에 동봉한다.** `erdd serve`는 `apps/web`의 빌드 산출물을 정적으로 내보내는데 설치본에는 `apps/web` 자체가 없다 — 그대로 두면 서버는 뜨는데 `/p/<id>`만 404를 내는, 원인을 짚기 어려운 상태가 된다. 그래서 게시 직전 `pnpm -C packages/cli run bundle:web`이 `apps/web/dist`를 `packages/cli/web/`로 복사하고 그 디렉터리가 `files`에 들어간다(5.2 MB · 101 파일). 번들 없이 팩하는 것은 `packages/cli` 의 `prepack` 가드가 종료 코드 1 로 막는다 — CI 밖의 손 게시까지 덮는 그물이다.
- **경로 해석은 저장소 배치(`apps/web/dist`)를 먼저 보고, 없을 때 동봉본(`<패키지 루트>/web`)으로 떨어진다.** 순서를 이렇게 정한 이유는 **개발 중의 잔재가 최신 빌드를 가리지 않게** 하는 것이다 — `packages/cli/web/`은 gitignore 라 `git clean -fd`로도 지워지지 않아서, 동봉본을 먼저 보면 게시 준비를 한 번 해 본 개발자는 그 뒤로 `apps/web`을 아무리 다시 빌드해도 계속 그 복사본을 보게 된다(표식 파일로 재현된 함정이다). 반대 방향의 사고는 성립하지 않는다 — **설치본에서 저장소 후보는 `<소비처>/node_modules/apps/web/dist`로 풀려 `node_modules` 안이라 존재할 수 없고**, 그래서 설치본은 언제나 동봉본을 쓴다.
- **게시 트리거는 `cli-v<SemVer>` 형태의 git 태그 하나뿐이다**(예: `cli-v0.1.0`). 브랜치 push·MR 로는 파이프라인이 아예 만들어지지 않는다 — 이 저장소에 CI 가 하나도 없던 상태에서 **게시에 필요한 최소한**만 둔 것이고, 평소 CI 를 돌릴지는 따로 결정한다. 파이프라인은 `verify`(typecheck · core·cli·**web** 테스트 · web 빌드 · 번들 복사) → `publish`(가드를 지난 뒤 게시) 두 단계이고, `publish`는 verify 가 만든 번들을 artifact 로 그대로 받는다(검증한 것과 게시하는 것이 같아야 한다). **`apps/web` 테스트가 verify 에 있는 이유**는 그 빌드 산출물 101개가 그대로 tarball 의 `web/`이 되어 소비처에서 서빙되기 때문이다 — 게시물의 대부분이 web 이라 그 줄이 없으면 무검증 게시 경로가 열린다.

⚠️ **`@erdd/core`는 버전이 그대로면 게시를 건너뛴다** — cli 만 고친 릴리스에서 409로 죽지 않게 한 의도된 동작이다. 다만 그 건너뛰기에는 조건이 붙는다: **직전 `cli-v*` 태그와 core 버전이 같은데 `packages/core`가 바뀌었으면 파이프라인이 실패한다.**(「버전이 같은가」를 함께 보지 않으면 core 를 올바로 올린 릴리스에서 publish 잡을 재시도할 때 오탐해 그 태그로는 영영 성공할 수 없다 — 재리뷰가 재현했다.) 「새 cli + 옛 core」가 조용히 게시되던 유일한 경로를 그 가드가 막는다 — **core 를 고쳤으면 `packages/core/package.json`의 `version`도 함께 올려야 한다**(→ [HANDOFF 4.1절](superpowers/HANDOFF.md#41-릴리스--사내-레지스트리에-cli-패키지를-게시한다)).
