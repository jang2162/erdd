...
- **원본 TypeScript 를 그대로 배포한다(빌드 산출물이 아니다).** 진입점의 shebang 이 `#!/usr/bin/env -S npx tsx`라 소비처에 `tsx`가 필요하지만, 그 대가로 번들링·소스맵·이중 진입점을 관리하지 않아도 되고 **저장소에서 직접 실행하던 형태와 게시본이 같은 코드**가 된다. CLI 는 사내 개발자가 쓰는 도구라 `tsx` 하나를 더 요구하는 비용이 작다.
- **웹 번들을 tarball 에 동봉한다.** `erdd serve`는 `apps/web`의 빌드 산출물을 정적으로 내보내는데 설치본에는 `apps/web` 자체가 없다 — 그대로 두면 서버는 뜨는데 `/p/<id>`만 404를 내는, 원인을 짚기 어려운 상태가 된다. 그래서 게시 직전 `pnpm -C packages/cli run bundle:web`이 `apps/web/dist`를 `packages/cli/web/`로 복사하고 그 디렉터리가 `files`에 들어간다(5.2 MB · 101 파일). **경로 해석은 설치본 배치(`<패키지 루트>/web`)를 저장소 배치(`apps/web/dist`)보다 먼저 본다** — 설치본에서 상위 디렉터리를 훑다가 남의 `apps/web/dist`를 잡는 일을 막기 위해서다. 대신 저장소에서 `bundle:web`을 한 번 돌리면 그 복사본이 우선하므로, web 을 고쳤는데 화면이 안 바뀌면 `packages/cli/web`이 남아 있는지 의심한다.
- **게시 트리거는 `cli-v<SemVer>` 형태의 git 태그 하나뿐이다**(예: `cli-v0.1.0`). 브랜치 push·MR 로는 파이프라인이 아예 만들어지지 않는다 — 이 저장소에 CI 가 하나도 없던 상태에서 **게시에 필요한 최소한**만 둔 것이고, 평소 CI 를 돌릴지는 따로 결정한다. 파이프라인은 `verify`(typecheck·core·cli 테스트·web 빌드·번들 복사) → `publish`(가드 3개 후 게시) 두 단계이고, `publish`는 verify 가 만든 번들을 artifact 로 그대로 받는다(검증한 것과 게시하는 것이 같아야 한다).

⚠️ **`@erdd/core`는 버전이 그대로면 게시를 건너뛴다** — cli 만 고친 릴리스에서 409로 죽지 않게 한 의도된 동작이다. 그래서 **core 를 고쳤는데 core 의 `version`을 올리지 않으면 cli 만 새로 나가고 core 는 옛 코드가 남는다.** 이것을 막는 가드는 없다.
