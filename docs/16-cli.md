...
- **원본 TypeScript 를 그대로 배포한다(빌드 산출물이 아니다).** 진입점의 shebang 이 `#!/usr/bin/env -S npx tsx`라 소비처에 `tsx`가 필요하지만, 그 대가로 번들링·소스맵·이중 진입점을 관리하지 않아도 되고 **저장소에서 직접 실행하던 형태와 게시본이 같은 코드**가 된다. CLI 는 사내 개발자가 쓰는 도구라 `tsx` 하나를 더 요구하는 비용이 작다. `tsx` 를 `dependencies` 로 선언해 두었지만 **그것으로 해결되는 것은 npm·yarn 평면 배치뿐이다** — pnpm 은 남의 의존성을 소비처 루트 `.bin` 에 걸지 않고 shebang 의 `npx` 는 소비처 cwd 기준이라, pnpm 소비처는 여전히 `tsx` 를 직접 넣어야 한다(실측). 두 패키지 모두 `engines.node: ">=22"` 로 Node 버전을 소비처에 강제한다.
...
⚠️ **`@erdd/core`는 버전이 그대로면 게시를 건너뛴다** — cli 만 고친 릴리스에서 409로 죽지 않게 한 의도된 동작이다. 다만 그 건너뛰기에는 조건이 붙는다: **직전 `cli-v*` 태그 이후 `packages/core`가 바뀌었는데 버전이 그대로면 파이프라인이 실패한다.** 「새 cli + 옛 core」가 조용히 게시되던 유일한 경로를 그 가드가 막는다 — **core 를 고쳤으면 `packages/core/package.json`의 `version`도 함께 올려야 한다**(→ [HANDOFF 4.1절](superpowers/HANDOFF.md#41-릴리스--사내-레지스트리에-cli-패키지를-게시한다)).
