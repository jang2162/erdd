...
```
core 877 · cli 284 · web 943 · server 209 · typecheck EXIT=0
```

「CLI DDL·DBML 내보내기/가져오기」 사이클에서 **core +18 · cli +50 · web −13** 이 됐다(직전 기준선은
`core 859 · cli 234 · web 956`). **web 이 줄어든 것은 회귀가 아니다** — `ddl-import-edits.test.ts`
14건이 `applyDdlImport` 와 함께 core 로 **옮겨 갔고**(그래서 core 의 +18 중 14 가 이동분이다) 그 자리에
「web 이 dagre 를 주입한다」 잠금 1건만 남겼다. cli 의 +50 은 신규 `export`(9) · `import`(16) ·
`roundtrip`(3) 과 `main`·`init` 배선 잠금들이다.
⚠️ **server 209 는 이번에도 재지 않았다** — `apps/server` 를 한 줄도 건드리지 않았다(`pnpm -r typecheck`
는 EXIT=0). 서버를 건드리는 다음 사이클은 격리 DB로 다시 재라.

「CLI 로컬 모드」 사이클에서 **core +11 · cli +79 · web +17** 이 붙었다(직전 기준선은
`core 848 · cli 155 · web 939`). 새 코드가 통째로 `packages/cli/src/local/**` 라 cli 가 가장 크게 늘었다.
⚠️ **server 209 는 이 사이클에서도 재지 않았다** — `apps/server` 는 `auth.me` **한 곳**만 바뀌었고
서버 스위트는 `DATABASE_URL` 이 필요해 워크트리에서 돌리지 않았다(`pnpm -r typecheck` 는 EXIT=0).
서버를 건드리는 다음 사이클은 격리 DB로 다시 재라.
