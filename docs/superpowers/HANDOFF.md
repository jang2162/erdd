...
**최종 갱신:** 2026-08-18 / **main HEAD:** `14a0de9`(머릿말 메타 구현 계획 — 이 사이클의 구현 커밋은 아직 병합 전) / **마이그레이션:** 0013까지(그룹 별칭 `model_table_groups.alias` — **테이블 물리명/논리명 형식 템플릿은 둘 다 마이그레이션 없음**)
...
```
core 834 · cli 145 · web 939 · server 209 · typecheck EXIT=0
```

「그룹·별칭 왕복 복원」 사이클에서 **core +29 · web +1** 이 붙었다(직전 기준선은
`core 805 · web 938`). **cli 는 무변경**이고 **웹 프로덕션 변경은 `ddl-import-edits.ts` 한 줄뿐**이다.
⚠️ **server 209 는 직전 기준선을 그대로 옮긴 값이고 이 사이클에서 재지 않았다** — `apps/server` 는
`ddl-import` 를 쓰지 않아(착수 전 전수 확인) 서버 스위트가 이 변경과 무관하고, 그래서 `. ./.env`
없이 core·web·typecheck 만 돌렸다. 서버를 건드리는 다음 사이클은 격리 DB로 다시 재라.

| 라운드 | core | web | 담은 것 |
|---|---|---|---|
| Task 1 `erdd:v2` 형식 | +5 | 0 | `name-meta` +5(v2 직렬화·파싱 · `g` 구획 · 마커 경계 검사). v1 케이스들은 **형태만** `{tables, groups:{}}` 로 바꿔 그대로 통과시켰다 |
| 수정 F(T1) | **0** | 0 | 잠기지 않던 두 줄(마커 경계 검사 · `g` 형태 검사)에 단언을 붙였다 — 기존 케이스를 좁힌 것이라 케이스 수가 그대로다 |
| Task 2 빌더 통합·그룹 적재 | +6 | 0 | `name-meta` +6(`buildNameMeta` 신설 — 싣는 기준이 「조합≠부분 **또는** 그룹 소속」으로 넓어진 것과 빈 별칭·빈 코멘트 키 생략). `ddl.ts`·`dbml.ts` 의 복제 빌더가 사라졌다 |
| 수정 F(T2) | +1 | 0 | `name-meta` +1(**빈 색**을 생략하는 분기. 픽스처의 그룹은 색이 늘 차 있어 기존 케이스가 반대 방향만 잠갔다 — 두 방향을 다 잠그려면 케이스가 둘이어야 한다) |
| Task 3 가져오기 그룹 복원 | +3 | +1 | `ddl-import` +3(머릿말 그룹 합류 · 속성 복원 · D7) · `ddl-import-edits` +1(새 그룹에 별칭이 실린다). **web 세 자리는 `pnpm typecheck` 로만 드러났다**(런타임은 전부 초록이었다) |
| 수정 F(T3) | +2 | 0 | `ddl-import` +2(그룹 이름의 **대소문자 왕복 보존** · 건너뛴 테이블이 그룹 멤버로 새지 않음). **프로덕션 무변경** |
| Task 4 충돌 키 확장 | +3 | 0 | `ddl-import` +3(충돌 키가 (그룹, 부분 이름) 이 된다). ⚠️ 계획서에 없던 테스트 1건이 빨개져 멈추고 물었다 — 픽스처가 D6 를 정면으로 맞은 것이라 **모델 쪽 테이블을 같은 그룹에 넣어** 고쳤다 |
| 수정 F(T4) | +3 | 0 | `ddl-import` +3(**블록 그룹 회귀** — 충돌 키의 그룹 소스가 머릿말뿐이면 DBML 블록이 말하는 그룹을 못 봐 같은 물리명이 둘 생긴다. 잠금 2건 + 구분자 NUL 잠금 1건) |
| Task 5 `group-conflict` | +4 | 0 | `ddl-import` +4(경고 3건 + 실증 중 찾은 무잠금 분기 1건 — 머릿말 별칭이 **빈 문자열**이면 경고하지 않는다) |
| 수정 F(T5) | +2 | 0 | `ddl-import` +2(경고 `target` 이 대문자 색인 키가 아니라 **모델의 원문 이름**이라는 잠금 · 기존 별칭이 빈 문자열일 때의 **다른 문구** 갈래) |

「세는 범위 + 부분 이름 잠금」 사이클에서 **core +1 · web +3** 이 붙었다(그때의 직전 기준선은
`core 804 · web 935`, 그 사이클 종료 시점이 `core 805 · web 938`). **cli·server 는 무변경.**
프로덕션 변경은 `ddl-import.ts` 한 줄뿐이라 대부분이 잠금이다.

| 라운드 | core | web | 담은 것 |
|---|---|---|---|
| A 세는 범위 | +2 | +1 | `ddl-import` +2(진짜 중복 CREATE TABLE 케이스 신설, 기존 왕복 테스트의 `skippedTables` 단언을 `[]` → `['TB_PRD_ORD']` 로 **갱신**) · `ddl-import-dialog` +1(요약이 그 건을 포함해 센다) |
| B 부분 유지 잠금 | 0 | +2 | `table-tree` +1 · `canvas` +1. **프로덕션 무변경** — 설계 D3 을 테스트로만 못 박았다 |
| 수정 F-1 | **−1** | 0 | 리뷰가 A-1 을 기존 (3) 단언과 **완전 중복**으로 실증(세 치환에서 항상 함께 빨감, A-1 만 빨간 치환을 구성할 수 없음)해 지웠다. 그 계약의 유일한 잠금이 (3) 이라는 것을 그 자리 주석에 명시했다 |
...
| 라운드 | core | 담은 것 |
|---|---|---|
| Task 1 메타 모듈 | +12 | `name-meta` +12(직렬화 3 · 파싱 9) |
...
| 수정 F2-(a) 문구 분기 | **0** | 「만들어질 이름이 겹친다」의 두 사유에 각자 다른 경고 문구를 낸다. **케이스 수는 안 바뀐다** — 기존 중복 케이스에 문구 단언을 더하고(`kind` 만 보고 있었다) 관찰 고정 케이스를 새 문구로 갱신한 것이라, 두 갈래를 서로 바꿔야 빨개진다 |
| 수정 F-4 형제 분기 | **0** | **브라우저 스모크에서 찾았다** — 같은 잘못된 문구가 형제인 `existing` 분기에 남아 있었고 **자기 덤프를 되읽는 가장 흔한 워크플로가 그쪽을 탄다**. 같은 방식으로 갈랐고 web 요약 줄 문구도 함께 고쳤다(web +0 — 기존 케이스의 정규식을 문구까지 좁혔다) |
...
수정 라운드에서 설정 미리보기 가드를 닫은 몫이다(F-2). **같은 라운드의 F-1 은 수치를 안 바꾼다** —
기존 4건의 픽스처만 강화해 구분력을 준 것이라 케이스 수가 그대로다. ⚠️ **server 수치는 워크트리 격리
...
| 수정 F-2 | 0 | 0 | +1 | 0 | `project-settings` +1(빈 템플릿 가드 — 두 사이클 연속 이월 해소) |
...
- **실증 수치를 보고할 때 「어느 범위로 돌렸는지」를 함께 적어라**(`-t` 필터 / 파일 하나 / 스위트 전체).
  범위가 다르면 **같은 훼손도 실패 개수가 달라져** 리뷰가 미잠금으로 오해한다. 실물: 논리명 사이클에서
  구현자가 접기 (a) 훼손을 「3 failed」로 보고했는데 `-t '빈 구간 접기'` 로 좁힌 값이었고, core 전체로는
  다른 describe 의 `접기 규칙을 물리명과 공유한다` 가 더해져 **4 failed** 였다. 미잠금이 아니라 측정
  범위 차이였고 리뷰어가 그 차이를 확인하는 데 시간을 썼다. 좁혀 돌리는 것 자체는 괜찮다 —
  **수치 옆에 범위를 적기만 하면 된다.**
...
- **v1 은 계속 읽는다. 그 하위호환의 잠금은 `ddl-import.test.ts` 의 v1 머릿말 케이스 3건이다** —
  「메타 키가 실제 이름과 안 맞으면 현행 동작으로 떨어진다」·「복원된 이름이 기존 테이블과 부딪히면
  건너뛴다」·「다른 그룹의 두 테이블이 같은 부분으로 복원되면 뒤엣것이 빠진다」. **세 케이스의 `-- erdd:v1`
  픽스처를 v2 로 「정리」하면 v1 경로가 통째로 무테스트가 된다.** 파서 단위의 v1 잠금은
  `name-meta.test.ts` 의 `parseNameMeta — v1 하위호환` 묶음이 따로 갖고 있다(둘 다 있어야 한다 —
  파서만 잠그면 `ddl-import` 가 v1 을 어떻게 쓰는지가 안 잠긴다).
...
- **머릿말 없는 조각 붙여넣기** — 설계 D1 이 수용한 대가다. 첫 줄 주석을 지우거나 `CREATE TABLE` 한
  ...
  사라진다. DBML 만 예외로, `TableGroup` 블록을 같이 붙여넣으면 그 블록이 말하는 그룹·색은 살아난다.
...
- **D7 은 머릿말이 이긴 그룹의 DBML 블록을 통째로 버린다.** 사람이 `TableGroup` 블록에 멤버를 손으로
  더해 뒀어도, 같은 이름의 그룹이 머릿말에 있으면 **그 멤버도 그 블록의 색도 사라진다**(멤버는 머릿말이
  말하는 것만, 색은 웹이 팔레트에서 고른다). 「머릿말이 이긴다」의 문자 그대로의 결과이고 우리 산출물
  에서는 둘이 늘 일치하므로 실사용 영향은 **사람이 손댄 파일에 한한다.** 합치는 쪽(멤버는 합집합, 속성은
  머릿말)이 더 친절해 보이지만 그러면 「우리가 쓴 것이 확실한 쪽을 믿는다」는 D7 의 근거가 사라진다.
- **도달 불가 방어 분기 둘이 안 잠겨 있다**(`ddl-import.ts` 8번 절의 머릿말 그룹 수집).
  ...
  **커버리지 구멍이 아니다:** `buildNameMeta` 는 테이블에 `g` 를 실을 때 **반드시** `groups[이름]` 도
  ...
  픽스처가 필요하다.
- **그룹의 화면 좌표는 복원하지 않는다.** 머릿말에 좌표를 싣지 않고, 적용부도 테이블의
  `groupPosition` 을 `null` 로 둔다(그룹 뷰 좌표는 그 뷰를 처음 열 때 계산된다). 그래서 같은 모델을
  내보냈다 되읽으면 **그룹 뷰의 배치가 원본과 다르다.** 좌표는 「이름을 되돌린다」는 머릿말의 목적과
  성격이 다르다 — 캔버스 좌표(`position`)도 머릿말에 없고 가져오기가 `computeAutoLayout` 으로 새로
  배치한다. 좌표 왕복을 다룰 때 둘을 같이 보면 된다.
