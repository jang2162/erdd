...
`dict-panel` 2 · `edit-panel` 2 다. ⚠️ **「방금 친 값」과 「재생성 한 번이 뮤테이션 한 건이다」는 둘 다 같은 것(draft 기반 읽기)을 잠근다.**
`regenerate` 가 draft 대신 커밋된 props 를 읽게 바꾸면 2건이 함께 빨개진다. **`preventDefault` 는 그
둘 중 어느 것도 잠그지 않는다** — 지우면 web 전건이 통과한다. 그 줄은 「포커스 유지」 3건이 잠근다.
