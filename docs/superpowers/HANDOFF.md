...
```
core 805 · cli 145 · web 938 · server 209 · typecheck EXIT=0
```

「세는 범위 + 부분 이름 잠금」 사이클에서 **core +1 · web +3** 이 붙었다(직전 기준선은
`core 804 · web 935`). **cli·server 는 무변경.** 프로덕션 변경은 `ddl-import.ts` 한 줄뿐이라
대부분이 잠금이다.
