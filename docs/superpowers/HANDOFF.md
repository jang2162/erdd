...
- **머릿말 복원 충돌로 건너뛴 테이블이 미리보기 요약의 「건너뜀 N개」에 안 잡힌다.**
  `apps/web/src/editor/ddl-import-dialog.tsx:120-121` 의 요약 줄은 `skippedTables` 만 세는데, 그
  배열에는 **모델의 기존 테이블과 부딪힌 쪽만** 들어간다. 머릿말이 서로 다른 두 이름을 같은 부분으로
  되돌려 뒤엣것이 빠지는 경로는 **경고 목록(`:123-131`)에만** 뜨므로 완전히 조용하지는 않지만,
  세는 자리와 알리는 자리가 갈라져 있어 놓치기 쉽다. **고친다면 갈림이 둘이다** — 그 경로도
  `skippedTables` 에 넣을지(그러면 요약 문구 「이미 있는 이름:」이 안 맞게 된다), 아니면 요약 줄이
  `table-conflict` 경고를 함께 세게 할지. web 을 건드려야 해서 core 안에서 끝낸 이 사이클 범위 밖으로
  뒀다.
- **컬럼 물리명 템플릿** — 요청 밖.
