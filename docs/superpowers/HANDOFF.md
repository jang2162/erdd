...
| **명명 입력 UI 개편** | 이름을 치는 자리(편집 패널 테이블·컬럼)에서 사전을 오가지 않고 이름을 완성하게 했다. 컬럼의 2열 가로 배치를 **세로**로 펴고(라벨 자리가 생겨 필수 표시가 붙는다), 재생성 버튼을 **입력란 안**으로 넣고 이름을 「물리명 재생성」·「논리명 재생성」으로 통일했다. ⚠️ **급소는 두 draft 를 한 컨테이너(`NamePair`)가 쥐는 것이다** — ↻ 는 *반대편 필드의 아직 커밋되지 않은 값*을 기준으로 삼아야 맞는데, draft 가 각 필드 안에만 있으면 그 값에 닿을 수 없다. 이것이 이월 결함(「버튼이 blur 커밋 전 값을 읽는다」)을 **실제로 닫는** 자리다(옛 코드는 비제어 인풋이라 친 값이 React 상태 어디에도 없었다). ⚠️ **`onMouseDown` 의 `preventDefault` 는 다른 것을 막는다 — 뮤테이션이 2건이 되는 것이다.** 없으면 `mousedown → blur(커밋 1건) → click(커밋 1건 더)` 로 Revision 2건 · undo 2회가 된다(값 자체는 draft 덕에 맞게 나온다). **설계·계획서는 이 두 줄이 같은 테스트에 잠긴다고 봤으나 실측은 갈렸다** — draft 를 커밋된 값으로 바꾸면 「방금 친 값」이, `preventDefault` 를 지우면 「재생성 한 번이 뮤테이션 한 건이다」가 빨개진다(둘 다 실증). 자동완성은 core `suggestCompletions` **하나**만 내려갔다 — 쿼리 산출이 `generatePhysicalName`·`restoreLogicalName` 과 **같은 분해 규칙**이어야 하고 `abbreviationIndex` 가 `naming.ts` 의 private 함수라 밖에서 재현할 수 없다. **용어만 입력 전체로 찾는다**(용어의 적용 규칙이 논리명 전체 완전일치라 꼬리 조각으로 찾으면 의미가 달라진다 — 그래서 용어 후보의 `start` 는 0 이다). ⚠️ **미등록 칩은 커밋된 값으로 계산한다** — draft 로 하면 타이핑 중 마지막 구간이 늘 미등록이라 칩이 깜박인다(**자동완성은 입력 중, 칩은 입력 후**로 역할을 가른다). 인라인 등록은 사전 화면의 일괄 등록과 **같은 `createWord` 경로**이고, 등록과 「반대편이 비면 채우기」를 한 producer 로 묶어 Revision 1건이다. 사전 화면의 미등록 항목은 **방향별 하위 탭 2개**로 갈리고 55줄 중복이던 두 섹션이 `direction` prop 하나로 합쳐졌다(그 통합이 `DEFAULT_NAMING_RULES` 하드코딩도 함께 닫았다). ⚠️ **컬럼의 「논리명이 용어와 완전일치하면 도메인도 함께 채운다」 규칙은 `applyNames` 안에 되살려야 한다** — 컬럼에만 있는 규칙이라 컨테이너가 모르고, 계획서가 그것을 옮겨 담지 않아 조용히 사라질 뻔했다(테스트가 없어 빨개지지도 않는다). **서버·CLI 변경 없음, 마이그레이션 없음** ([설계](specs/2026-08-14-naming-input-ux-design.md)) |
...
명명 입력 UI 개편 사이클에서 **core +11 · web +39**가 붙었다(직전 기준선은 `core 630 · web 800`이었다).
...
범위를 넘은 것이다. 내역은 `naming` 11(core) · `name-pair`(신규 파일) 27 · `dict-edits` 8 ·
`dict-panel` 2 · `edit-panel` 2 다. ⚠️ **`name-pair` 의 27건 중 배선 전체를 잠그는 것은 둘뿐이다** —
「방금 친 값」(draft 기반 읽기)과 「재생성 한 번이 뮤테이션 한 건이다」(`preventDefault`). 각각을
되돌려 빨개지는 것을 확인했고, **그 둘이 서로 다른 줄을 잠근다**는 것이 실증에서 드러났다(위 완료 표).
`dict-panel` 2건도 `candidates` 삼항을 뒤집어 실증했다.

⚠️ **설계가 "이 테스트가 이 줄을 잠근다"고 지목한 것을 그대로 믿지 마라.** 이 사이클의 설계·계획서는
`preventDefault` 를 지우면 「방금 친 값」이 빨개진다고 적었는데, 실제로는 **통과했다** — 제어 인풋의
draft 가 이미 친 값을 쥐고 있어 blur 가 먼저 커밋돼도 ↻ 는 같은 draft 를 읽기 때문이다. 지목이 틀려도
**실증 단계가 있었기 때문에** 드러났다. 지목을 검증 없이 옮겨 적으면 "잠겨 있다고 적힌 채 안 잠긴"
줄이 남는다.
