...
**최종 갱신:** 2026-08-15 / **main HEAD:** `e389934`(관계선을 컬럼 위치에 붙인다 병합 — ⚠️ 논리명 구분자 트랙 병합 후 갱신 필요) / **마이그레이션:** 0012까지(초대·재설정 링크에서 `invitations`·`password_reset_tokens` 추가)
...
>
> **명명 입력 UI 개편 이후 남은 두 묶음(실사용 마찰에서 나온 것):**
> - **A: 단축키 확장** — 메모 삭제/복사·관계선 삭제(`use-shortcuts.ts`). 지금 단축키가 테이블·컬럼에만
>   걸려 있어 메모·관계선은 마우스로만 지운다.
> - **B: 그룹 별칭 + 테이블명 형식 템플릿** — `TableGroupSchema` 에 필드 추가라 **마이그레이션 + 등록처
>   6~8곳**(3.2절 체크리스트)이 따라오고, 그보다 "저장된 물리명 vs 조합해서 보여 주는 물리명"이 갈리는
>   **새 개념**이 들어온다. 착수 전에 그 갈림을 먼저 확정해야 한다.
