...
**최종 갱신:** 2026-08-16 / **main HEAD:** `f2cadf9`(.env 로딩 drizzle 스크립트 병합) / **마이그레이션:** 0012까지(초대·재설정 링크에서 `invitations`·`password_reset_tokens` 추가)
...
**스키마 스냅샷 드리프트 (db 스크립트 사이클에서 관찰, 범위 밖으로 두었다)**
- **스키마를 하나도 안 고쳤는데 `pnpm db:generate` 가 마이그레이션을 하나 만든다.** `naming_rules`
  컬럼의 DB DEFAULT 는 `DEFAULT_NAMING_RULES`(`schema.ts:43`)인데, 거기 추가된 `logicalSeparator` 가
  **0012 스냅샷에는 없다** — 그래서 매번 `ALTER … SET DEFAULT` 하나가 생성물로 떨어진다(그 사이클에서는
  범위 밖이라 되돌렸다). **런타임 영향은 확인하지 않았다**(DEFAULT 만의 차이이고, 실제 삽입 경로가 값을
  넘기는지 여부는 이 사이클에서 보지 않았다). 다음에 스키마를 건드리는 사이클이 이 DEFAULT 를 같이
  담으면 정리된다.
