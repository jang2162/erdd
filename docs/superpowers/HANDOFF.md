...
```
core 834 · cli 151 · web 939 · server 209 · typecheck EXIT=0
```
...
- **`validate` 의 좌표는 디스크의 실제 파일이 아니라 `physicalName` 에서 재조립한 정규 경로다.**
  파일명을 손으로 바꿔 두면 좌표가 그 파일을 못 가리킨다. 물리명이 빈 테이블에서 **없는 경로**
  (`erdd/tables/.yaml`)를 내던 것은 막았지만(`unsafeFileName` 에 걸리면 좌표를 아예 안 낸다)
  뿌리는 남아 있다 — 제대로 고치려면 **읽은 파일의 실제 경로를 모델에 실어 와야 하고 그것은
  core 변경**이다.
- **core 가 `too-long`·`reserved` 경고 문구를 안 잠근다** — 그 문구를 부분 이름으로 되돌려도
  core 834 가 전부 초록이다(2026-08-20 리뷰 실측). CLI 쪽은 두 끝을 함께 단언하는 테스트로
  잠갔지만 core 자체에는 잠금이 없다.
...
- **D7 은 머릿말이 이긴 그룹의 DBML 블록을 통째로 버린다.** 사람이 `TableGroup` 블록에 멤버를 손으로
  더해 뒀어도, 같은 이름의 그룹이 머릿말에 있으면 **그 멤버도 그 블록의 색·코멘트도 사라진다**(멤버는
  머릿말이 말하는 것만, 속성은 머릿말의 `a`·`c`·`n` 이 정한다 — `ddl-import.ts:485` 의 `attrs?.c ?? null`.
  웹이 팔레트에서 고르는 것은 **머릿말에 `c` 가 없을 때뿐**인데(`ddl-import-edits.ts:49` 의
  `g.color ?? nextGroupColor(...)`), 웹에서 만든 그룹은 늘 팔레트 색을 받으므로(`group-edits.ts:38`)
  `c` 가 비는 경우가 사실상 없다). 「머릿말이 이긴다」의 문자 그대로의 결과이고 우리 산출물
