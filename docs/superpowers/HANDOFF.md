...
**최종 갱신:** 2026-08-16 / **main HEAD:** `f2cadf9`(.env 로딩 drizzle 스크립트 병합) / **마이그레이션:** 0013까지(그룹 별칭 `model_table_groups.alias`)
...
- **테이블명 형식 템플릿**(`TB_{그룹별칭}_{커스텀속성}_{물리명}`) — **별칭의 유일한 소비자이자 다음
  사이클의 주제다.** 지금 별칭은 입력만 되고 어디에도 안 나간다. 얹기 전에 「저장된 물리명 vs 조합해서
  보여 주는 물리명」을 먼저 정해야 한다 — DDL·Excel·DBML·역설계 왕복·중복검사가 전부 그 갈림에 걸린다.
- **중복 별칭 경고** — 템플릿이 없으면 두 그룹이 같은 별칭이어도 아무 문제를 일으키지 않아 판정 근거가
  없다. 만들면 새 `Warning['kind']` 등록처(3.14)가 따라온다. 템플릿 사이클에서 함께 본다.
- **별칭의 DBML·Excel·DDL 노출** — 쓰이는 곳이 없는데 산출물 포맷을 바꾸면 왕복 테스트만 흔들려 범위
  밖으로 뒀다. 템플릿이 물리명을 바꾸기 시작하면 그때 함께 본다.
