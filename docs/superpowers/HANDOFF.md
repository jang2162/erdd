...
**최종 갱신:** 2026-08-17 / **main HEAD:** `a3291c4`(그룹 별칭 병합) / **마이그레이션:** 0013까지(그룹 별칭 `model_table_groups.alias` — **테이블 물리명 형식 템플릿은 마이그레이션 없음**)
...
```
core 740 · cli 143 · web 926 · server 206 · typecheck EXIT=0
```
...
- **소비처는 테이블 물리명 형식 템플릿이다**(3.16). `{그룹별칭}` 변수로 조합에 들어간다.

### 3.16 테이블 최종 물리명 (`composeTablePhysicalName`)
...
- **논리명 템플릿** — 다음 사이클(설계 D6). **조합된 논리명이 용어 매칭에 쓰이는지가 선행 결정이다.**
  물리명은 용어 검사만 부분 기준으로 남기면 됐지만, 논리명은 용어 완전일치의 **입력 자체**라 그 판단이
  훨씬 넓게 번진다.
...
- **접기 규칙이 변수 뒤 리터럴을 통째로 지우는 것이 의도인지 재검토** — 지우는 단위가 「리터럴 토큰
  하나」라 `TB_{그룹별칭}_LOG` 에서 별칭이 비면 `_LOG` 까지 사라져 `TB_` 가 된다(`TB_` 는 유효
  식별자라 경고도 안 뜨고 DDL 로 나간다). 설계 3.2 의 규칙을 글자 그대로 따른 결과이고 설계의 예시
  4형이 전부 「변수 뒤 리터럴 = 구분자 하나」라 안 보였던 자리다. **동작은 그대로 두고**
  `docs/13-naming.md` 접기 규칙 절에 경계로 적어 뒀다 — 「구분자만 지운다」로 바꿀지는 다음 사이클 판단.
