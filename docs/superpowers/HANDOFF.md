...
### 3.17 내보낸 산출물의 첫 줄 주석 (`erdd:v1` 머릿말)

- **내보낸 산출물의 첫 줄 주석(`erdd:v1`)은 가져오기 계약의 일부다.** 지우면 왕복이 옛 동작으로
  돌아간다(조합된 이름이 통째로 부분이 된다). **새 내보내기 포맷을 추가하면 머릿말도 함께 실어야
  왕복이 닫힌다** — 직렬화·파싱은 `name-meta.ts` 하나가 갖고 있고 포맷마다 갈리는 것은 주석 접두
  (`--` / `//`)뿐이다.
...
- ⚠️ **실을 것이 없으면 줄 자체를 내지 않는다**(설계 D4). 판정은 「템플릿이 비었는가」가 **아니라**
  「조합 결과가 부분과 다른가」다 — 템플릿이 `{물리명}` 이라 결과가 같은 테이블도 걸러지고, 템플릿을
  안 쓰는 프로젝트의 산출물은 **한 글자도 안 바뀐다.** 그 무변경을 `ddl.test.ts` 의 전체 문자열
  `toBe` 비교가 함께 잠근다(실증: 그 `continue` 한 줄을 지우면 `-t '머릿말 메타'` 밖의 그 케이스까지
  빨개진다).
- ⚠️ **깨진 메타는 전부 `null` 이다. 예외를 던지지 않는다**(설계 D6). 깨진 JSON · 모르는
  버전(`erdd:v2`) · 형태가 다른 값 · 실제 이름과 안 맞는 키 — 전부 「머릿말이 없는 것」으로 떨어져
  옛 동작이 된다. 가져오기가 실패하면 안 된다.
- ⚠️ **`planDdlImport` 안에서 테이블 이름은 두 종류다. 어느 쪽인지 매번 정하라.**
  ...
  | 이름 충돌 판정(`existing` · 중복) | **복원된 부분 이름** | 실제로 만들어질 이름이 그것이다. 원문 이름으로 보면 `TB_MBR_ORD`→`ORD` 복원이 모델의 기존 `ORD` 와 부딪히는 것을 놓쳐 같은 물리명이 둘 생긴다 |
...
- ⚠️ **「만들어질 이름이 겹친다」의 사유는 둘이고 경고 문구를 갈라야 한다.** 원문 이름까지 같으면
  ...
  되돌린 것**이다(그룹이 갈라 원본에서는 중복이 아니었는데 가져오기가 그룹을 복원하지 않아 겹친다).
  ...
  - ⚠️ **형제 분기(`existing.has`)도 같은 이유로 갈라야 한다 — 여기가 훨씬 흔하다.** 자기 덤프를
    ...
    판정은 `restored = madeKey !== key`(머릿말이 이름을 되돌렸는가)다. 되돌렸으면 **원문 이름과
...
- **복원되는 것은 테이블의 물리명·논리명뿐이다.** 템플릿 자체·그룹 별칭·컬럼은 복원하지 않는다
  (설계 D2, 6절 이월).
...
- **템플릿·그룹 별칭 복원** — 범위 밖(설계 D2). 머릿말은 「어떤 최종 이름이 어떤 부분에서 나왔는가」만
  싣고 **어떤 템플릿이었는지는 안 싣는다.** 가져온 테이블은 템플릿이 없는 상태의 부분 이름으로
  들어오고, 그룹 별칭도 복원되지 않는다.
- **머릿말 복원 충돌로 건너뛴 테이블이 미리보기 요약의 「건너뜀 N개」에 안 잡힌다.**
  `apps/web/src/editor/ddl-import-dialog.tsx` 의 요약 줄은 `skippedTables` 만 세는데, 그 배열에는
  **모델의 기존 테이블과 부딪힌 쪽만** 들어간다. 머릿말이 서로 다른 두 이름을 같은 부분으로 되돌려
  뒤엣것이 빠지는 경로는 **경고 목록에만** 뜨므로 완전히 조용하지는 않지만, 세는 자리와 알리는
  자리가 갈라져 있어 놓치기 쉽다. **고친다면 그 경로도 `skippedTables` 에 넣을지, 요약 줄이
  `table-conflict` 경고를 함께 세게 할지가 갈림이다.**
  - ⚠️ **요약 문구 자체는 2026-08-19 에 고쳤다**(「이미 있는 이름:」 → 「이름이 겹침:」). 나열되는
    것이 **DDL 원문 이름**인데 머릿말이 되돌리면 부딪힌 이름은 그것과 다르므로, 존재를 단언하는
    옛 문구가 거짓이었다(브라우저 스모크에서 「이미 있는 이름: TB_invitations」 로 관측 — 모델에
    있는 것은 `invitations` 다). **`skippedTables` 의 값(원문 이름)은 그대로 뒀다** — 사용자가
    원문에서 찾는 좌표이고 경고 `target` 과 기준이 같다. 남은 것은 「세는 범위」뿐이다.
- **컬럼 물리명 템플릿** — 요청 밖.
