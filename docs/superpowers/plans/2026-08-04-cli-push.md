...
/**
 * 매달린 참조를 제거하고 무엇을 지웠는지 돌려준다. **model을 제자리에서 고친다** —
 * applyMerge가 자기가 만든 새 모델에만 쓴다.
 *
 * 필요한 이유: 서버가 pull 이후 추가한 관계가 로컬에서 지운 테이블을 가리킬 수 있다.
 * 그대로 두면 FK가 NOT DEFERRABLE이라 반영이 500으로 터진다.
 * 컬럼을 먼저 정리해야 인덱스·관계 검사가 정리된 결과를 본다.
 */
...
/** 서버 모델을 pull한 직후 상태를 만든다(트리 + base). */
async function seed(server: ProjectModel): Promise<void> {
  const { tree } = modelToFiles(server)
  await writeTree(dir, tree)
  await writeBase(dir, tree)
}

type StubOpts = { seq?: number; pushImpl?: (input: unknown) => Promise<{ seq: number }> }

function stub(server: ProjectModel, opts: StubOpts = {}): { client: ApiClient; pushCalls: unknown[] } {
  const pushCalls: unknown[] = []
  const client: ApiClient = {
    query: vi.fn(async (path: string) => {
      if (path === 'model.get') return { model: server, seq: opts.seq ?? 1 }
      if (path === 'project.get') {
        return { name: '커머스', dialects: ['postgresql'], namingRules: CONFIG.namingRules }
      }
      throw new Error(`unexpected query ${path}`)
    }) as ApiClient['query'],
    mutate: vi.fn(async (_path: string, input: unknown) => {
...
```bash
git add packages/cli/src/plan.ts packages/cli/src/commands/push.ts \
        packages/cli/src/commands/push.test.ts packages/cli/package.json pnpm-lock.yaml
git commit -m "$(cat <<'EOF'
feat(cli): erdd push
...
`packages/cli/src/commands/diff.test.ts` — `push.test.ts`의 `CONFIG`·`beforeEach`/`afterEach`·`seed`·`StubOpts`/`stub` 블록을 **그대로 복제**해 쓴다(두 파일이 독립적으로 읽히도록 의도적으로 반복한다). import는 다음으로 시작한다:
