...
| 프로시저 | 권한 | 하는 일 |
|---|---|---|
| `create` | Org Owner/Admin (`requireOrgManager`) | §3.3 검사 → 같은 (email, org) 미사용 초대 만료 → 발급. **평문 토큰은 여기서만 반환** |
| `listForOrg` | Org Owner/Admin | 대기·만료·사용됨 목록(평문 토큰 없음) |
| `revoke` | Org Owner/Admin | `expiresAt`을 현재로 당긴다. 조건부 UPDATE로 이미 사용된 것은 못 건드린다 |
| `peek` | **공개** | 토큰 → `{ email, orgName, orgRole }`(조직 없는 초대면 둘 다 `null`). `assertLive`. **`userRole`은 내보내지 않는다** — 수락자가 알 이유가 없고 화면에도 쓰지 않는다 |
