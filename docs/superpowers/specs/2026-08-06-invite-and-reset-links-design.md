...
| 갈래 | 무엇 | 화면 |
|---|---|---|
| 종료성 | `BAD_REQUEST`(없는·기한이 지난·이미 사용된 토큰) · `CONFLICT`(이미 가입한 이메일) | 폼을 지우고 사유 + "관리자에게 문의해 새 링크를 받으세요" |
| 비종료성 | 그 밖 전부 — 네트워크 단절, 5xx, `PRECONDITION_FAILED` | **폼을 유지**하고 인라인 오류 + 재시도 |

**허용 목록이고, 모르는 오류는 비종료성으로 떨어진다.** 오분류의 대가가 한쪽으로만 크기 때문이다.
...
판정은 `apps/web/src/lib/link-error.ts`의 `terminalLinkReason` 한 곳에 있다. 초대 화면의 `peek`은
