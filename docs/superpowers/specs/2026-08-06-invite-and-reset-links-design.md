...
**관련 기획:** [docs/18-account.md](../../18-account.md) "추후 검토"의 셀프 가입·초대·비밀번호 재설정 메일
...
된다 — **구현자가 실측해 더 단순한 쪽을 고르고 근거를 보고한다**). 지켜야 할 것은 둘이다:
**개인 조직 생성이 계정 생성과 갈라지지 않을 것**, **이메일 정규화가 이 함수 안에서만 일어날 것**.
...
- `users.create`(`initialPassword`) → **`users.invite`**: 이메일·이름·서비스 역할을 받아 계정 없이
  **초대만** 만든다. **`orgId`는 `null`이다**(§4) — 관리자가 사람을 시스템에 넣는 것과 조직에 넣는
  ...
  - `name`은 초대 시점에 관리자가 적지만 **수락 화면에서 본인이 고칠 수 있다**(관리자가 적은 것은
    기본값이다). 초대 행에 `name`을 두지 않고 `accept`가 받는 이름을 쓴다 — 그래야 초대 행이
    "누구에게 어떤 자격을 준다"만 담고 프로필 정보를 들고 있지 않는다.
...
둘 다 토큰이 죽었을 때(만료·사용됨·없음) **같은 화면에서 사유를 보여주고 관리자에게 문의하라고
안내한다.** 사유별로 다른 문구를 쓰되 "토큰이 존재하지 않음"과 "만료됨"을 구분하지 않는다(존재 오라클).
...
- **토큰이 페이지 URL 경로에 남는다.** `/invite/:token` · `/reset/:token`(§6.1)의 경로 자체가 토큰이므로
  **브라우저 히스토리**와 그 페이지가 보내는 **같은 출처 Referer**에 들어간다. `peek`을 mutation으로
  둔 것(§3.5)은 API 요청 URL만 덮는 별개 조치이고 이 문제는 해결하지 않는다. 다룬다면 Task 4가
  마운트 직후 `history.replaceState`로 경로에서 토큰을 지우는 방식이다(토큰은 이미 컴포넌트 상태에
  들어와 있으므로 동작에는 지장이 없다). 지금은 링크 자체가 관리자→본인 직접 전달이라 위험이 낮다고
  보고 이월한다.
