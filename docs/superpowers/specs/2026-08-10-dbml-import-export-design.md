...
> **알려진 한계(고치지 않는다):** 설명이 손글씨 JSON으로 **끝나면** 그것을 커스텀 꼬리로 오탐한다
> (예: 설명 `기본값은 {"a":"b"} 이다`가 그 자리에서 끝나는 경우). **옛 규칙("마지막 `{`부터")도
> 똑같이 오탐한다** — 리뷰어가 옛 규칙을 복원해 돌려 결과가 같음을 실증했으므로 아래 수정이 만든
> 회귀가 아니라 **두 규칙이 공유하는 한계**다. 값이 전부 문자열인 평범한 객체로 끝나는 설명은 드물고,
> 오탐해도 값은 커스텀 항목 정의를 못 찾아 `unknown-custom-field` 경고로 드러난다.
...
> ⚠️ **두 함수는 정확한 역이어야 한다**(리뷰 m-5, 2026-08-11 수정). 이스케이프를 한 겹이라도 덜
> 풀면 백슬래시가 **왕복마다 두 배로 누적된다** — 한 번만 돌려 보면 "한 번 늘어난 값"이 그럴듯해
> 보여 눈에 띄지 않으므로, **두 번 연속 왕복시키는 테스트**로 잠근다. 표현식(백틱) 경로도
> 마찬가지이고, 거기서는 표현식이 백슬래시로 끝나면 닫는 백틱을 렉서가 먹어 **테이블이 통째로
> 사라진다**(§4.1의 문자열과 같은 결함 클래스).
