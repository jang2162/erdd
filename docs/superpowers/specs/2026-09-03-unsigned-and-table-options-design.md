...
**① 정수 표시폭(`int(10) unsigned`·`int(11)`)을 이번에 함께 고칠까 — 사용자 판단이 필요하다.**
→ **함께 고친다(추천).** 근거: MySQL 5.7 의 `mysqldump`·`SHOW CREATE TABLE` 이 내는 표준 모양이고,
...
닫힌다. **다만 브리프가 정한 범위(부호 없음·테이블 옵션) 밖이므로 §10 에도 적었다 — 빼기로 하면
이번 사이클은 `int unsigned`(8.0 모양)만 지원하고 5.7 모양은 오늘과 같이 `unknown-type` 으로 남는다
(회귀는 아니다).**
...
1. **정수 표시폭이 미지 타입이 된다.** `int(11)`·`bigint(20)`·`smallint(5)` → `unknown-type`
   (실측 §3.4 가). MySQL 5.7 덤프 전체가 여기 걸린다. **→ §9 ① 이 「이번에 함께」를 추천한다.**
