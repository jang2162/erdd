...
2. 이동 집합의 bbox를 구한다.
3. `delta.x = targetMaxX + GAP - moveMinX` (`GAP = 60`) → 붙일 자리의 x가 정해진다.
4. `delta.y`는 `targetMinY - moveMinY`에서 **출발**하되, 그 자리가 **이동 대상이 아닌 테이블**과
   `GAP` 안으로 붙으면 빈자리를 만날 때까지 **아래로 민다**(6.3).
5. 모든 이동 대상에 같은 delta를 더한다 → **상대 배치 보존**.

bbox 계산은 `group-nodes.ts`의 것과 같은 로직이므로 **`tableBounds(model, tables, counts?)` 헬퍼를 뽑아
양쪽이 공유한다**(HANDOFF 3.5 "중복 헬퍼 통합" 선례).

### 6.1 `groupPosition`은 null로 초기화한다

테이블은 좌표를 둘 갖는다 — 전체 뷰의 `position`과 그룹 뷰 전용 `groupPosition`(`nodes.ts:31`이
`groupPosition ?? position`으로 폴백). 이전 그룹 뷰에서 잡아 둔 좌표는 새 그룹에서 의미가 없으므로
`groupPosition`을 `null`로 되돌린다. 그러면 새 그룹의 그룹 뷰에서 전체 뷰 좌표로 폴백해 자연스럽게
자리를 잡는다.

**비우는 대상은 이동 대상 전원이다 — 이미 대상 그룹에 있던 것도 포함한다.** 여러 그룹에 걸친 선택을
그중 하나로 옮기면(정상 시나리오다) 일부는 그룹이 바뀌지 않는데, 그때 그것만 옛 `groupPosition`을
남기면 **함께 옮긴 것들이 그룹 뷰에서 갈라진다** — 하나는 옛 좌표에, 나머지는 폴백 좌표에 놓인다.
전원 비워야 모두 폴백해 나란히 선다.
...
`planGroupMove`는 이동 집합을 기준 bbox에서 **스스로 제외**하므로 **호출 순서에 무관하다** — `m`을
넘기든 `next`를 넘기든 반환값이 같다(그래서 호출 순서는 테스트로 관찰할 수 없다. 어느 모델을 넘겨도
web 스위트가 전부 통과한다). `m`을 넘기는 것은 의도를 드러내는 관례이지 정확성 요건이 아니다.

⚠️ **정확성을 지탱하는 것은 `planGroupMove` 안의 `movingIds` 필터다.** 그 필터가 막는 것은 잘못된
호출이 아니라 **정상 시나리오**다 — 여러 그룹에 걸친 선택을 그중 일부가 이미 속한 그룹으로 옮길 때,
그 테이블이 자기 자신의 기준 bbox가 되어 이동 집합 전체가 자기 오른쪽으로 밀리는 것을 막는다. 이
경로는 일괄 패널 드롭다운에서 그대로 도달하고, `group-move.test.ts`의 `이동 대상은 대상 그룹의 기준
bbox에서 제외한다`가 **단독으로** 잠근다(필터만 지우면 그 테스트가 실패한다). 호출 순서를 지켜도
이 필터를 대신하지 못하므로 **필터를 군더더기로 보고 지우면 안 된다.**

### 6.3 "그룹 오른쪽"이 비어 있다는 보장은 없다 — 겹침을 피해 아래로 민다

**초판 설계의 구멍이다.** 3의 `delta`만으로 놓으면 기준이 **대상 그룹 멤버뿐**이므로, 그룹 **밖**
테이블이 이미 그 자리에 있으면 그 위에 그대로 얹힌다. 2026-08-11 브라우저 스모크가 실제로 잡았다 —
대상 그룹 bbox의 `maxX + GAP`에 미분류 테이블이 앉아 있자 두 테이블의 `position`이 **`(1000,0)`으로
완전히 일치**해 한쪽이 화면에서 완전히 가려졌다. 사이드바에는 남아 있으므로 사용자에게는 "테이블이
사라졌다"로 보인다. 조작으로 만든 상황이 아니라 그냥 쓰다가 두 번 나왔다.

**규칙:** 옮긴 집합의 bbox는 **이동 대상이 아닌 어떤 테이블과도 `GAP` 안으로 붙지 않는다.**
`GAP(60) > GROUP_PAD * 2(56)`이므로 이 규칙 하나로 **그룹 색상 영역끼리의 포개짐도 함께 사라진다.**

**푸는 방향은 아래(+y)다.** x는 `대상 그룹 maxX + GAP`으로 **고정**한다 — 옆으로 밀면 D3의 "새 그룹
옆에 붙인다"가 무너져 옮긴 테이블이 그룹에서 멀어진다. 아래로 미는 것은 그 의도를 지킨 채 자리만
비켜 주는 유일한 방향이다.

**집합은 bbox 하나로 다룬다.** 테이블마다 따로 빈자리를 찾으면 겹침은 사라지지만 **상대 배치가
깨진다**(D3이 잠근 것이다). 감싸는 사각형으로 판정하므로 결과가 보수적일 수는 있어도 겹침은 남지
않고, 집합 전체가 같은 delta로 움직이므로 상대 배치가 그대로다.

**결정적이다.** 후보 y는 장애물의 `maxY + GAP` 값만 취하고 그 값은 **단조 증가**한다. 같은 모델·같은
선택이면 항상 같은 좌표가 나온다(랜덤·시간 의존 없음).

**규모 대응 — 여기가 함정이다.** 겹침 회피는 이동 대상이 **아닌** 테이블 전부의 높이를 알아야 하는데,
높이는 컬럼 수에서 나온다. 테이블마다 `model.columns`를 filter하면 O(테이블 × 컬럼)이고, 최종 리뷰가
2,000테이블/3만컬럼에서 드롭 1회 **229ms**를 실측한 그 비용이 그대로 곱해진다. 그래서 둘을 쓴다.

- **컬럼 계수 1회 계산**(`columnCounts`): 컬럼 전체를 한 번만 훑어 `Map<tableId, count>`를 만들고
  `tableBounds`·장애물 판정이 돌려 쓴다. O(테이블 × 컬럼) → **O(테이블 + 컬럼)**.
  `tableBounds(model, tables, counts?)`의 3번째 인자가 이것이다(생략하면 내부에서 1회 만든다 —
  그룹마다 부르는 `buildGroupNodes`도 덕을 본다).
- **x 띠로 장애물 선별**: 세로로만 미므로 후보 x 구간에 걸치지 않는 테이블은 영원히 장애물이 아니다.
  테이블 1회 순회로 걸러 내면 세로 훑기는 그 띠 안(보통 한 자릿수)에서만 돈다.

### 6.4 테스트로 잠근 것

- 겹침 회피 자체 — 스모크의 최소 재현(대상 그룹 오른쪽에 그룹 밖 테이블이 이미 있는 모델)이 픽스처다.
  좌표 고정 단언과 **"결과가 이동 대상이 아닌 테이블과 겹치지 않는다"** 를 함께 본다.
- 밀린 뒤에도 **상대 배치가 보존**된다.
- 장애물이 **연달아** 쌓여 있으면 마지막 것 아래까지 내려간다(한 번만 밀면 두 번째 위에 얹힌다).
- **이동 대상 자신은 장애물이 아니다** — 이미 목적지에 서 있는 테이블을 옮기면 좌표가 그대로다.
  장애물 목록에서 이동 집합을 빼지 않으면 자기 자신을 피하느라 근거 없이 내려간다.
