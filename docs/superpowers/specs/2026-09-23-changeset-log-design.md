...
이미 지웠다). 같은 순번 안에서는 테이블 물리명 → 컬럼 순서 → 이름의 코드 단위 사전순. 개명(인덱스·테이블·
컬럼)은 그 위에 **의존성 순**으로 둔다 — 새 이름이 아직 개명 전인 다른 개명의 옛 이름이면 그쪽을 먼저 낸다
(`MEMBER→USER` 가 `ACCOUNT→MEMBER` 보다 앞선다). 인덱스 이름은 PostgreSQL 에서 스키마 전역이라 인덱스
개명은 테이블을 가로질러 정렬한다. **위에서 아래로 옮겨 쓰면 의존성이 맞는다.** 순환 개명(맞바꾸기)만
예외로, 기록은 `rename` 여러 줄로 나오고 SQL 에서는 임시 이름을 거쳐야 한다(「알려진 한계」).
(최종 리뷰 반영: 처음 설계는 drop/rename index 를 한 단계에 섞고 개명을 이름순으로만 두어, 같은 이름의
인덱스 삭제+개명과 개명 사슬에서 SQL 이 이름 충돌로 실패했다.)
...
- **없는 대상·풀리지 않는 참조도 같은 논리로 경고하고 그 요소만 건너뛴다.** `@id` 대상이 없는 `drop`·
  `rename`·`alter table`(블록 전체)·`modify column` 은 건너뛰고(두 브랜치가 같은 컬럼을 지웠다),
  이름 참조가 없거나 둘 이상이면 `add index`·`add foreign key` 는 추가하지 않고, `primary key` 는 풀리는
  이름만 남기고, `after`·`position` 은 그 컬럼을 테이블 끝에 둔다(한쪽이 바꾼 이름을 다른 쪽이 참조했다).
  남은 차이는 미기록 변경으로 드러나고 새 기록으로 정리한다 — 그 새 기록의 정확성은 자기검증이 지킨다.
  (최종 리뷰 반영: 처음 설계는 이것들을 4.5 의 오류 행에 두었는데, 공유된 두 기록이 파일명 순의 우연으로
  어긋나면 지울 수도 고칠 수도 없어 재생이 영구히 멈췄다.)
...
### 4.5 오류 — 멈추고 파일·줄을 알린다

| 경우 | 동작 |
|---|---|
| 문법 오류 | `〈파일〉 〈줄〉: 〈무엇을 기대했는데 무엇이 왔나〉` |
| `format` 이 이 CLI 가 아는 것보다 크다 | 「더 새 ERDD 로 만든 기록입니다 — CLI 를 올리세요」 |
| 재생 실패 — 이미 있는 `@id` 에 `create`/`add`, 한 `alter table` 블록 안의 순서 모순(두 컬럼이 같은 자리, 닫힌 `after` 사슬). 없는 대상·풀리지 않는 참조는 오류가 아니라 경고다(4.3) | 파일·줄과 `@id` |
