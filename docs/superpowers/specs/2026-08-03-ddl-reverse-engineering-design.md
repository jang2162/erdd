...
- **논리명은 전부 맞아야 제안.** 물리명의 모든 토큰이 사전에 매칭될 때만 논리명을 제안하고, 하나라도 실패하면 물리명을 그대로 두고 경고한다. 한글·영문이 섞인 반쪽짜리 논리명을 만들지 않는다.
...
1. 물리명 전체가 어떤 용어의 `physicalName`과 일치하면 그 용어의 `logicalName`을 쓴다(용어사전이 "논리명 전체가 완전일치할 때 우선 적용"이므로 역방향도 용어가 우선).
2. 아니면 명명 규칙의 구분자로 쪼개 각 토큰을 단어사전의 `abbreviation`과 역매칭한다. **모든 토큰이 매칭되면** 논리명을 이어붙인다.
3. 하나라도 실패하면 물리명을 논리명으로 두고 미매칭 토큰을 경고한다.

경고 목록이 곧 사전에 채워 넣을 일감이 된다. 테이블명·컬럼명 모두 같은 규칙을 쓴다.
