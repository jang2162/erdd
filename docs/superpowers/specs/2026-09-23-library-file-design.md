...
- 헤더 해석·행 검증은 기존 `planDictImport` 를 **빈 모델**에 대고 재사용한다. 결과 draft 를 라이브러리 payload 로
  옮기고, 용어의 `domainName` 은 같은 파일 도메인의 파일 안 id 로 잇는다.
- **Excel 에는 커스텀 항목 시트가 없으므로 결과에 `customFields` 키를 두지 않는다**(부재 = 말하지 않음).
- **없는 컬럼은 말하지 않은 것이다.** 기존 Excel 가져오기의 원칙과 같다 — 영문명 컬럼이 없는 파일이 기존
  영문명을 지우면 안 된다. 그래서 변환 결과는 항목마다 **실제로 있던 필드 집합**을 함께 싣고, 2절의 갱신 판정이
  그 필드만 비교·덮어쓴다. 라이브러리 파일(YAML)은 항목이 완전값이라 이 집합이 곧 전체 필드다.
- 셀 문자열을 읽는 층(파일 바이트 → `RawSheet[]`)만 웹(`excel-file.ts` 의 `readDictSheets`)과 CLI(노드용 리더)가
  따로 갖는다. 그 뒤는 전부 core 다.
...
- **부분 필드(Excel)** — 항목에 필드 집합이 실려 있으면 그 필드만 비교하고, `update` 는 기존 payload 에 그 필드만
  덮은 값을 쓴다. `add` 는 빠진 필드를 기본값으로 채운다(기존 Excel 가져오기의 `draft = 기본값 + patch`).
...
- **`resource.library.list`** 를 `apiProcedure` 로 연다 — `erdd library list` 가 프로젝트 없이 부른다.
...
- **파일 구독은 `dict push` 대상이 아니다.** 지정하면 「파일에서 받은 사전은 올릴 수 없습니다 — 서버에 연결된 뒤
  구독의 file 을 지우세요」(`2`). 인자 없는 `dict push` 는 파일 구독을 건너뛰고 알린다.
...
- **core `library-import.test.ts`** — 같은 라이브러리 id 면 id 매칭·다르면 이름 매칭, id 매칭이 자리를 먼저 차지한다,
  **재매핑 뒤 같으면 `unchanged`**(다른 라이브러리에서 온 파일이 전부 `update` 가 되지 않는다), `stale` 은 파일
  ...
  건드리지 않는다, `expectedStateHash` 가 입력 순서와 무관하다.
