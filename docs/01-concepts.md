...
4. **승격(반대 방향)** — 프로젝트에서 다듬은 항목을 조직(또는 전역) 리소스로 올린다. 라이브러리에 쓰기 권한이 있는 사람(조직은 Org Owner/Admin, 전역은 서비스 관리자)은 프로젝트 화면에서 **직접 승격**한다. 권한이 없는 Project Editor는 **승격을 요청**하고, Org Owner/Admin이 조직 화면에서 항목 단위로 승인·반려한다 — 요청은 항목 묶음이며 한 번 처리하면 닫힌다. 요청 대상은 조직 라이브러리뿐이다(전역은 서비스 관리자가 직접 승격). 승격하면 그 항목의 상류가 대상 라이브러리로 바뀐다(항목당 상류는 항상 하나다).
...
| AccessToken (`access_tokens`) | CLI 인증용 **개인 액세스 토큰(PAT)** | 발급한 User 참조, 토큰 이름, 해시(평문은 발급 시 한 번만 표시), 마지막 사용 시각, 폐기 시각 |

AccessToken 은 **사용자 범위**다 — 프로젝트 범위가 아니고 자체 권한 컬럼도 없다. 권한은 토큰을
발급한 사용자의 조직·프로젝트 역할에서 그대로 파생되므로 새 권한 축이 아니고, **만료도 없고 폐기만
가능하다**(→ [16-cli](16-cli.md)).
...
| TableGroup | 테이블 그룹 | 이름, 별칭(물리명 조합용 짧은 식별자 — 영문·숫자·밑줄 대문자, 선택 입력), 색상, 설명, 소속 테이블 목록, 그룹 뷰 배치 좌표(테이블별) |
