...
이름이면 그 항목이 먼저다(`MEMBER -> USER` 가 `ACCOUNT -> MEMBER` 보다 앞선다). 인덱스 이름은
PostgreSQL 에서 스키마 전역이라 인덱스 개명은 테이블을 가로질러 정렬한다. **위에서 아래로 SQL 로
옮기면 의존성이 맞는다** — 예외는 순환 개명(맞바꾸기)뿐이다(「알려진 한계」). 이름은 그 문장 시점의
이름이다(`alter table` 머리는 개명 뒤 이름, `rename index … on T` 의 `T` 는 개명 전 이름).

- ⚠️ 삭제를 개명·추가 뒤로 옮기면 「지운 컬럼과 같은 이름의 새 컬럼」·「지운 테이블(인덱스) 이름으로
  개명」이 SQL 에서 이름 충돌을 낸다.
- ⚠️ 개명을 이름순으로만 두면 개명 사슬이 아직 비지 않은 이름으로 바꾸려다 SQL 에서 실패한다. 재생은
  `@id` 라 기록 자체는 맞으므로 **마이그레이션을 돌릴 때에야** 드러난다. `roundtrip.test.ts` 가 매 단계
  문장의 이름 충돌을 검사해 잠근다.
...
FK·인덱스·PK·`after` 의 참조만 그 시점의 이름으로 푼다. 대상이나 참조가 풀리지 않으면 「경합은 경고다」.
...
여러 기록이 서로 다른 전제로 쓰여(브랜치 병합) 생긴 어긋남은 **경고하고 그 요소만 건너뛴다.** 남은
차이는 미기록 변경으로 드러나고, 그것을 새 기록으로 만드는 것이 정리 방법이다 — 새 기록의 정확성은
「생성 시 자기검증」이 지킨다.
...
**여전히 멈추는 것** — 한 기록 안의 모순이거나 같은 기록이 두 번 들어온 것이라, 넘기면 기준선이 거짓이 된다.

- 문법 오류, 이 ERDD 가 아는 것보다 새 `format`
- 이미 있는 `@id` 로 생성(`create table`·`add column`·`add index`·`add foreign key`)
- 한 `alter table` 블록에서 두 컬럼이 같은 자리를 가리키거나 `after` 가 서로를 가리킨다

- ⚠️ 이 어긋남을 오류로 멈추면 재생이 **영구히** 멈춘다. 두 기록이 모두 공유된 뒤라 지울 수도 고칠 수도
  없고, 모델을 고쳐도 기록은 그대로다. 성공 여부도 두 기록의 파일명 순(시각의 우연)에 달린다.
- ⚠️ 건너뛴 문장은 그 기록을 보고 쓴 마이그레이션에는 **이미 들어 있다**(두 브랜치가 같은 컬럼을
  지웠으면 두 마이그레이션이 모두 지운다). 경고가 뜨면 두 마이그레이션이 실제 DB 에서 돌 순서로 맞는지
  확인한다.
...
| 경우 | 이유 |
|---|---|
| 기록 파일이 깨졌다(문법 오류·새 `format`)거나 재생이 멈췄다(「경합은 경고다」의 「여전히 멈추는 것」) | 기준선을 모르는 채 차이를 내면 거짓 기록이다 |
...
뒤 기록이 그 파일·줄에서 경고를 내며 기댈 대상이 없는 문장을 건너뛰고, 기준선이 다른 사람의 것과
갈라진다.
...
- **순환 개명**(두 컬럼·테이블·인덱스의 이름 맞바꾸기 등)은 `rename` 여러 줄로 나오고 어떤 순서로도
  SQL 에서 바로 되지 않는다 — 임시 이름을 거쳐야 한다.
- 밖(터미널·다른 탭·`git pull`)에서 기록 파일만 늘어난 경우는 웹 탭에 이벤트가 없다 — 탭을 다시 열어야
  보인다.
- 서버 모드에는 없다.
