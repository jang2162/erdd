...
## 잡 스크립트가 git 을 부르려면 `safe.directory` 예외가 먼저다

러너가 만든 빌드 디렉터리는 **소유자가 잡 스크립트를 돌리는 사용자와 어긋난다.** git 은 그런 저장소를
`dubious ownership` 으로 거부한다(종료 코드 `128`). 러너는 **자기가 부르는** git 에만 예외를 붙이므로
그 설정은 잡 스크립트 안의 git 에 오지 않는다. 그래서 publish 잡의 **첫 단계**가 이것이고,
git 을 쓰는 어떤 단계보다 앞이어야 한다.

```bash
git config --global --add safe.directory "$CI_PROJECT_DIR"
```

- **`--global` 을 빼지 마라.** 저장소 로컬 설정은 그 저장소를 열 수 있어야 쓸 수 있는데, 막혀 있는
  것이 바로 그 「열기」다. 컨테이너는 잡마다 새로 뜨므로 전역 설정이 그 잡 밖으로 새지 않는다.
- **경로를 하드코딩하지 마라.** `$CI_PROJECT_DIR` 을 쓴다.
- **git 을 쓰는 잡을 새로 만들면 그 잡에도 같은 줄이 필요하다.** 공유 `before_script`(`.node` 앵커)가
  아니라 publish 잡의 `script` 안에 둔 것은, 거기 넣으면 git 을 부르지 않는 verify 에까지 붙기
  때문이다.

⚠️ **없으면 잡 안의 git 이 하나도 돌지 않는다.** 실패를 그대로 두는 단계(태그 fetch)는 시끄럽게
죽지만, **실패를 삼키는 곳은 「git 이 안 돌았다」와 「찾을 것이 없다」를 구분하지 못한다** —
아래 core 버전 가드가 정확히 그 자리다. 가드가 발동해야 할 때 조용히 통과하고, 로그에는
「직전 cli-v* 태그가 없어 … 건너뜁니다」만 남아 **원인이 태그 쪽으로 잘못 읽힌다.**
...
⚠️ **이 판정은 저장소에 `cli-v*` 태그가 전부 있어야 성립한다.** GitLab 러너는 태그 파이프라인에서
**그 태그의 ref 하나만** 가져온다. `GIT_DEPTH: "0"` 은 커밋 **깊이** 제한만 푸는 것이라 다른 태그를
가져오지 않는다 — **둘 다 필요하다.** 그래서 publish 잡이 `safe.directory` 예외 **바로 다음**,
가드보다 **앞**에서 태그를 따로 fetch 하고, **못 가져오면 릴리스를 죽인다. `|| true` 로 넘기게 고치지
마라** — 태그가 없는 상태의 「직전 태그 없음」은 첫 릴리스와 구분되지 않아, 가드가 발동해야 할 때
조용히 통과한다. 그 조용한 통과가 이 가드가 막으려는 사고 그 자체다.
...
- **러너 환경에 의존하는 것들은 파이프라인이 실제로 돌 때만 확인된다.** `corepack` 활성화가 사내
  ...
  `npm i -g pnpm` 은 **같은 네트워크를 타므로 대안이 아니다**), 태그 fetch 가 러너가 남긴
  자격증명으로 실제로 도는지, `git describe --match 'cli-v*'` 가 detached HEAD 태그 체크아웃에서
  같게 도는지, `pnpm -C apps/web test` 가 러너에서 타임아웃 없이 끝나는지가 그것이다.
  **태그 fetch 는 fetch 직후에 찍히는 `cli-v*` 목록으로 판정한다** — 이번 태그 하나뿐이면 자격증명이나
  refspec 쪽이고, 여럿이면 들어온 것이다.
