...
가져오지 않는다 — **둘 다 필요하다.** 그래서 publish 잡의 **첫 단계**가 태그를 따로 fetch 하고,
**못 가져오면 릴리스를 죽인다. `|| true` 로 넘기게 고치지 마라** — 태그가 없는 상태의 「직전 태그
없음」은 첫 릴리스와 구분되지 않아, 가드가 발동해야 할 때 조용히 통과한다. 그 조용한 통과가 이 가드가
막으려는 사고 그 자체다.
...
- **러너 환경에 의존하는 것들은 파이프라인이 실제로 돌 때만 확인된다.** `corepack` 활성화가 사내
  ...
  `npm i -g pnpm` 은 **같은 네트워크를 타므로 대안이 아니다**), publish 잡 첫 단계의 태그 fetch 가
  러너가 남긴 자격증명으로 실제로 도는지, `git describe --match 'cli-v*'` 가 detached HEAD 태그
  체크아웃에서 같게 도는지, `pnpm -C apps/web test` 가 러너에서 타임아웃 없이 끝나는지가 그것이다.
