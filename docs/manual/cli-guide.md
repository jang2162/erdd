...
저장소 안에서 혼자 그리고 싶을 때 쓴다. 명령 옵션의 상세는 [6.8](#68-erdd-serve) 에 있다.
...
**방언은 파일에서 정한다.** 로컬 모드에 방언 편집 화면은 없다(웹에도 원래 없다 — 프로젝트를 만들 때
정해지는 값이다). `erdd.config.yaml` 의 `dialects` 를 직접 고치면 파일 감시가 그것을 반영한다. 명명
규칙은 브라우저의 「설정」에서 고칠 수 있고, 저장하면 이 파일에 되쓰인다.

**브라우저에서 무엇이 되고 무엇이 없나.**

| 그대로 되는 것 | 없는 것 |
|---|---|
| 에디터 전부 — 테이블·컬럼·관계·인덱스·메모·그룹·그룹 뷰·자동 정렬·단축키·클립보드 | 로그인·계정 설정·액세스 토큰 화면 |
| 실행 취소/다시 실행, 모델 검사(경고 전부) | 홈·조직·프로젝트 목록·관리자·초대 |
| DDL·DBML 가져오기/내보내기, 이미지·Excel 내보내기, 사전 Excel 업로드 | 참여자(presence)·실시간 커서·사용자 메뉴 |
| 도메인·단어·용어·커스텀 항목 관리 | 「버전」의 **「이력」 탭**(「스냅샷」·「비교」는 그대로 있다) |
| 스냅샷 생성·복원 — `.erdd/snapshots.json` 에 담긴다 | 공용 리소스 라이브러리·승격 요청·프로젝트 멤버 |

로그인 화면이 뜨지 않고, `http://127.0.0.1:4300/` 은 곧바로 에디터로 넘어간다.
...
**파일이 깨지면 편집이 잠긴다.** `erdd/` 안의 YAML 이 파싱·스키마·무결성 검사에서 걸리면 마지막
정상 모델을 그대로 둔 채 상단에 빨간 배너(`파일을 읽을 수 없어 편집이 잠겼습니다 — <경로>: <사유>`)가
뜨고 하단 바가 「읽기 전용」으로 바뀐다. 파일을 고치면 자동으로 풀린다. 편집을 계속 허용하면 **성한
화면 상태가 깨진 파일을 덮어써 손으로 고치던 내용이 사라진다.** 명명 경고·필수 커스텀 항목 같은
것은 오류가 아니라 그대로 「모델 검사」에 뜬다.
...
- ⚠️ **로컬로 시작한 프로젝트를 서버로 옮기는 전용 명령은 없고, 연결 설정을 적는 것만으로는 부족하다.**
  ...
  pull을 실행하세요` 로 멈춘다. 서버에 프로젝트를 만들고 `erdd pull` 로 기준선을 받은 뒤 `push` 해야
  하는데, **그 `pull` 이 로컬 파일을 서버(빈) 상태로 덮어쓰므로** 커밋해 둔 `erdd/` 를 git 에서
  되살려 얹은 다음 `push` 한다. 좌표·메모(`layout.yaml`)는 서버로 올라가지 않는다(→ [8절](#8-파일에-담기지-않는-것)).
...
- ⚠️ **이 파일이 깨져도 편집은 잠기지 않는다.** 스키마 파일이 아니므로 파싱에 실패하거나 좌표가
  숫자가 아닌 항목은 **그 항목의 배치만 잃고**(격자로 떨어진다) 모델은 그대로 열린다.
