...
`--version`·`-v` 는 **명령 자리(첫 인자)에 올 때만** 버전으로 받는다(`--json` 은 앞뒤 어디에 와도 된다).
명령 뒤에 적은 `-v` 는 버전이 아니다 — `erdd status -v` 는 `-v` 를 무시하고 status 를 그대로 돌고,
`erdd export -o -v` 는 `-v` 라는 파일에 쓴다. `--help` 와 함께 주면 `--help` 가 이긴다.
웹 화면 상단 워드마크 옆에 뜨는 버전도 같은 값이다.
...
| `--version`, `-v` | 버전 문자열만 stdout 에 낸다(`--json` 이면 `{"version": …}`). 종료 코드 `0`. 명령 자리(첫 인자)에서만 받는다 — 명령 뒤의 `-v` 는 버전이 아니다. |
...
- **`--include-stale`** — 「오래된 파일」 항목도 파일 값으로 덮는다. 서버가 더 앞서 나간 값을 되돌리는
  것이므로 기본은 꺼져 있다.
- **파일이 잘못됐으면 거절한다**(`1`) — 파일 검사는 CLI 안에서 끝나므로 가져오기 반영
  (`resource.library.import`)은 부르지 않는다. **다만 `--library` 는 이미 라이브러리 목록을 조회한
  뒤에 파일을 검사한다** — 「서버를 아예 안 부른다」는 뜻이 아니다(조회 없이 곧장 검사하는 것은
  `--create --scope global` 뿐이다). 라이브러리 파일이면
  `라이브러리 파일 오류 N건 — 아무것도 반영하지 않았습니다` 뒤에 문제 줄, Excel 이면
  `Excel 오류 N건 — 아무것도 반영하지 않았습니다` 뒤에 시트·행과 함께.
- `--library` 로 지정한 라이브러리에 쓸 권한이 없으면 거절한다(`1`, `<이름> 에 쓸 권한이 없습니다`) —
  이 판정도 라이브러리 목록을 조회한(서버를 부른) **뒤** 로컬에서 하고, 가져오기 반영은 부르지 않는다.
