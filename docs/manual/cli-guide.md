...
**목차** — [1. 개요](#1-개요) · [2. 설치와 실행](#2-설치와-실행) · [3. 토큰 발급과 연결](#3-토큰-발급과-연결) · [4. 일상 워크플로](#4-일상-워크플로) · [5. 파일 구조와 포맷](#5-파일-구조와-포맷) · [6. 명령 레퍼런스](#6-명령-레퍼런스) · [7. 동기화와 충돌](#7-동기화와-충돌) · [8. 파일에 담기지 않는 것](#8-파일에-담기지-않는-것) · [9. AI 에이전트와 함께 쓰기](#9-ai-에이전트와-함께-쓰기) · [10. 자동화와 CI](#10-자동화와-ci) · [11. 문제 해결](#11-문제-해결) · [부록: 검증 상태](#부록-검증-상태-2026-08-14)
...
`serverUrl`·`projectId` 가 없거나 `dialects`·`namingRules` 의 형태가 어긋나면 명령이 종료 코드 `1`
로 멈춘다.
...
| | |
|---|---|
| 전용 옵션 | `--server <url>` · `--token <token>` · `--project <id>` |
...
파일은 스키마의 **의미 정보만** 담아 diff 를 깨끗하게 유지한다. 다음은 파일에 없고, `push` 도
건드리지 않는다(서버에 있던 값이 그대로 보존된다).

| 담기지 않는 것 | 어디서 다루나 |
|---|---|
| 테이블 배치 좌표(캔버스 위치, 그룹 뷰 좌표) | 웹 에디터 |
| 메모(note) | 웹 에디터 |
| 도메인·단어·용어·커스텀 항목의 **공용 리소스 출처(origin)** | 웹의 공용 리소스 화면 |

에이전트가 "좌표를 옮겨 달라", "메모를 남겨 달라"는 요청을 파일에서 처리하려 하면 안 된다 —
그런 정보는 여기에 없다.
...
**기준** — `packages/cli`(`main.ts`, `commands/*`, `plan.ts`, `tree.ts`, `client.ts`, `config.ts`,
`output.ts`), `packages/core/src/file-format.ts`, `apps/server/src/routers/*`(토큰 허용 프로시저).
