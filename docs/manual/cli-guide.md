...
하고(값이면 서버 모드, `null` 이면 로컬 모드 → [3.4](#34-로컬-모드--서버-없이-쓰기)), 워크플로와
금지 사항 중 갈리는 것을 모드별로 갈라 적는다. 로컬 전용 프로젝트에 깔려도 에이전트가 `pull`·`push`
를 시도해 종료 코드 `1` 로 막히는 일이 없다.
...
- **하지 말 것(공통)** — 기존 `id` 수정·삭제, 복사한 파일에서 `id` 를 안 지우기, 테이블 파일 이름
  직접 변경, `.erdd/` 편집.
- **하지 말 것(모드별)** — 좌표·메모는 서버 모드에서는 파일에서 찾지 않고, 로컬 모드에서는
  `erdd/layout.yaml` 에 있되 **손으로 고치지 않는다**(`erdd serve` 화면에서 옮긴다). `id` 발급 주체는
  서버 모드가 서버, 로컬 모드가 CLI 의 로컬 store 다. 공용 리소스 `origin` 은 서버 모드에만 해당한다.
