...
⚠️ **탭을 여러 개 열면 다른 탭은 편집을 따라오지 않는다.** 파일이 **밖에서** 바뀌는 것(에이전트·
`git pull`·에디터)은 모든 탭이 함께 받지만, **브라우저에서 한 편집**은 그것을 낸 탭에만 보인다 —
자기 편집을 되돌려 받으면 드래그하는 탭이 계속 튀기 때문에 걸러 낸다. 다른 탭은 다음 외부 변경 때
맞춰지고, 그 전에 맞추려면 새로고침한다. 로컬 모드는 한 사람이 쓰는 것을 전제한다.

**파일이 깨지면 편집이 잠긴다.** `erdd/` 안의 YAML 이 파싱·스키마·무결성 검사에서 걸리면(**메모가
담긴 `layout.yaml` 도 포함이다** → [5.7](#57-erddlayoutyaml--배치-좌표와-메모)) 마지막
